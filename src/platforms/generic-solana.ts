...
export interface GenericSolanaPlatformConfig {
  id: string;
  name: string;
  hostPattern: string[];
  hostnames: string[];
...
export class GenericSolanaPlatform implements IPlatform {
  readonly id: string;
  ...
  constructor(config: GenericSolanaPlatformConfig) {
    this.id = config.id;
    ...
    this.linkAddressPatterns = [...(config.linkAddressPatterns ?? []), ...EXPLORER_ADDRESS_PATTERNS];
  ...
  protected extractAddressFromDocument(): string | null {
    for (const anchor of Array.from(document.querySelectorAll<HTMLAnchorElement>('a[href]'))) {
      const address = this.extractAddressFromText(anchor.href);
      if (address) {
        return address;
      }
    }
  ...
  protected extractAddressFromText(value: string | null | undefined): string | null {
    ...
    return extractFirstSolanaAddress(value);
  }
